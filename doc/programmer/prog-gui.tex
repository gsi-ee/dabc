...
Thats how the class must look like:
{\small \begin{verbatim}
public class MyPanel  extends xPanelPrompt
...
(we do not provide a command formatting function):
{\small \begin{verbatim}
...
This is called once after constructor. Here we have to setup all panels.
...
frame=new xInternalCompound(name,graphIcon, 0, layout, xSet.blueD());
}
\end{verbatim}
}
...
System.out.println(cmd+":"+prompt.getText()+" "+check.isSelected());
}
else if ("button".equals(cmd)) {
System.out.println(cmd+":"+prompt.getText()+" "+check.isSelected());
}
else if ("check".equals(cmd)) {
System.out.println("Data server "+check.isSelected());
if(check.isSelected()){
...
With the checker we toggle the \class{xState} state.
The \class{xDimParameter} param to be toggled we will find in the next.
...
for(int i=0;i<vipar.size();i++){
    ...
    if(p.isRate()){ // with this, we take all meters
        meter=new xMeter(xMeter.ARC,
                pname,0.0,10.0,xMeter.XSIZE,xMeter.YSIZE,xSet.blueD());
        metpan.addGraphics(meter,false); 
        brows.addInfoHandler(vipar.get(i),
...
Now the callback function \func{infoHandler} which is called each time.
{\small \begin{verbatim}
...
We provide a little extra class implementing the \func{infoHandler} function.
