...
\item  The user code shall not directly access the 
      ...
      This method provides a blocking access to a memory pool, i.~e.~ the \func{MainLoop()} 
...
\item  Class \class{dabc::ModuleAsync} has no methods, which can block thread.
...
\item  In virtual \func{ExecuteCommand()} method one can implement 
...
One only adds \member{fCnt} member to count direction for output of next buffer. 
...
Here \keyw{while} loop can wait infinite time until output port cannot accept new buffer
and during this time complete thread will be blocked. 

While both processing methods are the same in the example,  
therefore one can implement \func{ProcessUserEvent} instead:  
