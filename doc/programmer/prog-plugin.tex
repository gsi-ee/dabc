...
\item  Instead of implementing \func{MainLoop()} (or \func{ProcessUserEvent()}, resp.) 
...
To manage the \class{LocalTransport}, the \class{dabc::Manager} always has instance of \class{dabc::LocalDevice} class.
It can be accessed via \func{dabc::mgr()->FindLocalDevice()} call. 
...
It is not always necessary to create a user-specific \class{Device} for a user written \class{DataTransport}, since the standard \class{LocalDevice}
can be used if it is only required as owner for the transport objects.
In this case, the factory method \func{CreateTransport()} 
should be provided  already in the user \class{Factory} (see section \ref{prog_plugin_factory}). 
...
provides the factory method \func{CreateTransport()} (see section \ref{prog_plugin_device_device}). This can instantiate the
\class{DataTransport} with a back reference to the responsible \class{Device}. 
