...
Creates thread for processor \func{proc} and assigns processor to this thread.
...
\item[\em Command* SetCmdReceiver\small (Command* cmd, const char* itemname)] :
\item[\em Command* SetCmdReceiver\small (Command* cmd, Basic* rcv)] :
\item[\em Command* SetCmdReceiver\small (Command* cmd, const char* nodename, const char* itemname)] :
\item[\em Command* SetCmdReceiver\small (Command* cmd, int nodeid, const char* itemname)] :
\index{Manager interface !SetCmdReceiver()}
...
or destroyed ($event=parDestroy=2$), resp. Pointer \func{par} should be used to access parameter
...
unexports (argument \func{reg} \keyw{false}) a command definition object
