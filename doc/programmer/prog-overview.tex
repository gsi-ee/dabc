...
Allocated memory areas are kept in \class{dabc::MemoryPool}
...
For \class{dabc::ModuleSync} \index{Core classes !dabc::ModuleSync}   , function \func{MainLoop()} may wait for buffers 
...
%\begin{compactdesc}
\begin{description}
\item[\class{dabc::Basic}] : \index{Core classes !dabc::Basic} The base class for all objects to be kept in \dabc~ collection (e.~g.~ \class{dabc::Folder}).
...
Subclass of \class{dabc::WorkingProcessor} (and by that also of \class{dabc::CommandReceiver}) 
