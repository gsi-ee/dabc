...
\item  The manager defines interface methods to the control system. 
      ...
      These methods must be implemented in subclass of \class{dabd::Manager} that 
...
\item  Instead of implementing \func{MainLoop()}, other virtual 
      ...
      implemented that are implicitely called by the superclass \func{MainLoop()}.
\item  The special base classes may provide additional 
      methods to be used for data processing.    
\end{compactitem}
\subsection{Device and transport}
...
\item  To implement a simple user defined transport 
      ...
      there is another base class \class{dabc::DataTransport} (subclass of \class{dabc::Transport}). 
...
\end{compactitem}
\subsection{Application plug-in}
The specific application controlling code is defined in 
   the \class{dabc::ApplicationPlugin}.   
\begin{compactitem}[$\circ$]
\item  The manager has exactly one application plug-in. 
      The user must provide a \class{dabc::ApplicationPlugin} subclass.
\item  The \class{dabc::ApplicationPlugin} is instantiated and registered on 
...
\item  The user may implement virtual methods \func{UserConfigure()} ,  
      ...
      \class{dabc::ApplicationPlugin} subclass. These methods are executed by the 
...
\item  The methods of the application plug-in should use 
      ...
      implement factory methods \func{CreateModule()} and \func{CreateDevice()} straight in his/her 
      \class{dabc::ApplicationPlugin} subclass.
\item   For special DAQ topologies (e.g. Bnet), the framework offers 
      implementations of the \class{dabc::ApplicationPlugin} containing the 
      generic functionality (e.g. \class{bnet::WorkerPlugin}, \class{bnet::ClusterPlugin}). 
...
\subsection{Core system}
  Defines all base classes and interfaces, like 
   ...
   \item \class{dabc::Buffer}
   \item \class{dabc::Device}
   \item \class{dabc::Transport}
   \item \class{dabc::Command}
   \item \class{dabc::Parameter}
...
\subsection{Control system}
   ...
   monitoring and modification. Interface to the \strong{Core system} is 
   implemented by subclass of \class{dabc::Manager}.       
\subsection{Configuration System}
   Depends on the \strong{Core system}; it 
   may use functionalities of the control system. Implements the 
   connection of configuration parameters with a database 
   (i.e. a file in trivial case). Interface to the \strong{Core system} is defined 
   by subclass of \class{dabc::Manager}. 
\subsection{Transport packages}
   One or more special transport packages: Depends on the \strong{Core system}. 
   It may depend on an external framework. Implements \class{dabc::Device} and 
   \class{dabc::Transport} classes for specific data transfer mechanism, e.g. 
   \strong{verbs} or \strong{tcp/ip socket}. Each transport package provides a 
...
\subsection{Bnet package}
   Depends on the \strong{Core system}. Implements special 
   ...
   worker nodes (\class{bnet::WorkerPlugin}) and 
   controller nodes (\class{bnet::ControllerPlugin}).    
\subsection{Application packages}
   Depend on the \strong{Core system}; may depend 
   ...
   modules that actually run the DAQ. May implement \class{dabc::Device} 
   and \class{dabc::Transport} classes for special data input or output. May provide 
   a factory to create a specific module or device by class name. 
   Provides implementation of \class{dabc::ApplicationPlugin} that defines the 
   application set-up; this may be a subclass of specific existing 
   application plug-ins (e.g. subclass of \class{bnet::WorkerPlugin}). 
...
\item \strong{Bnet} 
\item \strong{Transport packages} for \strong{verbs} and \strong{socket} 
\ebul
...
\item  Simple \ABB~ readout (this example also contains 
   a separate ransport package implementing the \class{dabc::Device} class for \ABB)
...
\subsubsection{Bnet package}
   Ready
\subsubsection{Application packages}
\begin{compactitem}[$\circ$]
...
\end{compactitem}
      ...
      one \class{dabc::ApplicationPlugin} object to define the state machine transition actions.
\end{compactenum}
\item[\class{dabc::Factory}] : Factory plug-in for creation of modules and devices.
\item[\class{dabc::ApplicationPlugin}] : Subclass of \class{dabc::WorkingProcessor} 
