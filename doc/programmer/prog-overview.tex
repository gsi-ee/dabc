...
This thread is used for manager commands execution, parameters timeout processing and so on. 
...
Represents a command object. A command 
   ...
   (i.e. module , manager, or device). See chapter (PUT REF TO SERVICES HERE)
...
\item[\class{dabc::WorkingProcessor}] :  \index{Core classes !dabc::WorkingProcessor} 
   ...
   In a special mode a processor can also run its own main process thread.
   %main loop.  
...
Subclass of \class{dabc::Module}; defines interface for an 
   ...
   but the user \strong{must} avoid any polling loops, waiting for availible resources - 
...
\figpng{emptyfig}{Simplified {\em core} classes diagram}{htb}{0}{.6}
...
\figpng{emptyfig}{Simplified {\em Bnet} class diagram}{htb}{0}{.6}
