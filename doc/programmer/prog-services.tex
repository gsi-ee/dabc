...
\figpng{emptyfig}{Schema of buffers with segmentts and memory pool with blocks and subblocks}{htb}{0}{.6}
...
It is also possible to create a mere reference \class{Buffer} object that uses
...
next event from the queue will be delivered to the \class{WorkingProcessor} by calling virtual method \func{dabc::WorkingProcessor::ProcessEvent()}. 
...
Some \class{WorkingProcessors} 
may require to be invoked not only by events, but also after specified time intervals.

Method \func{dabc::WorkingProcessor::ActivateTimeout()} 
prepares the thread to execute \\ \func{dabc::WorkingProcessor::ProcessTimeout()}
...
With each \class{dabc::SocketProcessor} a socket descriptor is associated which can
...
implemented with base class \class{dabc::WorkingProcessor}. 
...
\class{dabc::WorkingProcessor} inherits from \class{dabc::CommandReceiver},
...
In the \dabc~ subclasses of \class{dabc::WorkingProcessor}, like \class{dabc::Module}, \class{dabc::Application}, the custom commands will be executed 
...
Method \func{Execute()} can only check if a command is executed succesfully or not,
as it has a boolean return value. There are advanced methods \func{ExecuteInt()}, and
\func{ExecuteStr()}, which return the result of a command execution as integer, or string value, resp. They will deliver the final value of the command parameter which is specified
