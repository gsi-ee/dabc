...
created by the device. If the \class{Transport} implementation 
shall be able to block (e.~g.~ on socket receive), there can be only 
one transport for this thread. 

A \class{Device} instance usually represents an I/O component (e.~g.~ network card); 
there may be more than one \class{Device} instances of the same 
...
In contrast to the \class{Manager} implementation that defines a framework control system (e.g. DIM, EPICS), the \class{Application} defines the experiment specific behaviour of the DAQ.
...
The \strong{Core system} package defines all base classes and interfaces
