...
\lsubsection[MBS launch panel]{user:launchMbs}{\mbs\ launch panel}
\figpng{user-gui-pan-mbs}{MBS launcher.}{htb}{0}{0.6}
Fig. \paref{fig:user-gui-pan-mbs} show the panel to be used to control
a standard \mbs.
...
\item[MbsScript]: \mbs\ script to be executed via \keyw{rsh} (see button below).
An execution script located relativ to \mbs\ path.
Script \verba{script/remote\_exe.sc}
sets all \mbs\ related variables and executes a program given as argument
on user path. Arguments:
\bcir
\item \mbs\ system path (inserted by GUI). Script sets \keyw{MBSROOT} to this.
\item \mbs\ user path (inserted by GUI). Script changes directory to this
\item Program name.
\item \mbs\ path of program. Script executes \verba{m\_launch} which forks
...
\subsubsection[MBS launcher buttons]{\mbs\ launcher buttons}
...
\lsubsection[Combined DABC and MBS launch panel]{user:launchDabcMbs}{Combined \dabc\ and \mbs\ launch panel}
...
\subsubsection[Combined DABC and MBS launcher buttons]{Combined \dabc\ and \mbs\ launcher buttons}
\icon{fileclose} Close window.\\
\icon{savewin} Save panel settings, see \paref{user:guiSaveRestore}.\\
