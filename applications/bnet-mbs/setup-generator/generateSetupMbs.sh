...
if [ $# -lt 2 ] ; then
    ...
    if [[ "$2" -gt 0 ]]; then
        ...
        READERTEMP=`echo bnetReadoutTemp.xml`   
    else
        READER=`echo dabc::xd::Worker`    
        READERTEMP=`echo bnetWorkerTemp.xml`   
    fi 
    ...
    ############# eventbuilders for asymmetric setup:
        for NODENAME in $BUILDERNODELIST;
            ...
            cat  bnetEventbuilderTemp.xml | awk -v rep=$NODENAME -v str=XDAQNODE '{ gsub(str,rep) ; print }' \
    ...
    let "INSTCOUNTA=0" 
    echo  > $TEMPNAME
    cat  bnetControllerTemp.xml | awk -v rep=$CONTNODE -v str=CONTROLLERNODE '{ gsub(str,rep) ; print }'\
            | awk -v rep=$CONTROLLER -v str=CONTROLLERCLASS '{ gsub(str,rep) ; print }'\
