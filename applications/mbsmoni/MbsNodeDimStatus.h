...
public:
  ...
  void MD_update(char *);
private:
  DimService * MD_add_histo(char *name, dabcHistogram **histo, int chan, float min, float max, char *color, char *lett, char *cont);
  DimService * MD_add_rate(char *name, dabcRate *rate, char *color, char *text, int mode, float ul);
  DimService * MD_add_state(char *name, dabcState *state, int value, char *color, char *text);
  DimService * MD_add_info(char *name, dabcInfo *info, int value, char *color, char *text);
  DimService * MD_add_value(char *name, char *type, int size, unsigned int *value);
  void MD_set_info(dabcInfo *info, int value, char *color, char *text);
  void MD_set_state(dabcState *state, int value, char *color, char *text);
  ...
  char cPrefix[64];
  char cServerName[64];
  int iRemote,iTagOffset,iCommandIndex;
  int *piContinue;
  int fileOn;
  s_daqst *ps_daqst;
};
