...
p1name=new String("MbsMonitor");
back = new Color(0.0f, 0.2f, 0.0f); // overall background
tooltip=new String("Launch Mbs panel");
...
xSet.setColorBack(back);
    ...
    tim=new xTimer(MbsListener,false);
...
// Add prompter lines
    ...
    prompt1.setActionCommand("MbsCommand");
...
// Build button line
    ...
    addButton("mbsShell","Rsh Node -l Username MbsCommand",workIcon,this);
...
for(int i=0;i<vipar.size();i++){
    ...
    if(xip.isRate()){ // with this, we take all meters
        ...
        if(full.indexOf("RunMode")<0){
            if(xip.getName().equals("RunStatus"))
...
System.out.println("MbsPanel: "+e.getActionCommand());
boolean demo=check1.isSelected();
//-----------
...
else if ("mbsShell".equals(e.getActionCommand())) {
        mbsshell.rsh(formMbs.getMaster(),xSet.getUserName(),prompt1.getText(),0L);
}
else if ("mbsStart".equals(e.getActionCommand())) {
if(mbsCommand != null) {
