   ...
   class ModuleSync : public Module {
      private:
         ...
         bool WaitItemEvent(double& tmout,
                            ModuleItem* item = 0,
                            uint16_t *resevid = 0,
                            ModuleItem** resitem = 0) throw (Exception);
      ...
      protected:
         ...
         /** Set if commands should be executed synchronous im main loop */
         void SetSyncCommands(bool on = true) { fSyncCommands = on; }

         /** Returns true if commands should be executed synchronous im main loop */
