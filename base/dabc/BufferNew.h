...
namespace dabc {
   ...
   class BufferNew : public Reference {
      ...
      inline Reference GetPool() const;
       ...
       *  ...
       */
      BufferNew HandOver() { return Take(); }
      ...
      bool Append(BufferNew& src, bool moverefs = true) throw();
       ...
       * \param[in] moverefs  if true, references moved to the target and source buffer will be emptied
       * \returns false if operation failed, otherwise true */
      bool Prepend(BufferNew& src, bool moverefs = true) throw();
       ...
       * \param[in] moverefs  if true, references moved to the target and source buffer will be emptied
       * \returns false if operation failed, otherwise true */
      bool Insert(BufferSize_t pos, BufferNew& src, bool moverefs = true) throw();
      ...
      BufferSize_t CopyFrom(const BufferNew& srcbuf, BufferSize_t len = 0) throw();
      ...
      static BufferNew CreateBuffer(BufferSize_t sz) throw();

      /** This static method create Buffer instance, which contains pointer on specified peace of memory
       * Therefore it can be used in standalone case */
      static BufferNew CreateBuffer(const void* ptr, unsigned size, bool owner = false) throw();
   };
