...
namespace dabc {
   ...
   class CommandsSet : public Worker {
      public:
         CommandsSet(ThreadRef thrd, bool parallel = true);
         virtual ~CommandsSet();

         /** Set default receiver for commands in the set.
          * By default, manager is receiver of commands */
         void SetReceiver(Worker* proc);
         ...
         void Add(Command cmd, Worker* recv = 0, bool do_submit = true);
      ...
      protected:
         ...
         struct CmdRec {
            ...
            CmdRec() : cmd(), recv(0), state(0) {}

            CmdRec(const CmdRec& src) : cmd(src.cmd), recv(src.recv), state(src.state) {}

            ~CmdRec() { reset(); }

            CmdRec& operator=(const CmdRec& src)
            {
               cmd = src.cmd;
               recv = src.recv;
               state = src.state;
