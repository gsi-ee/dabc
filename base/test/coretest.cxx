...
class TestModuleAsync : public dabc::ModuleAsync {
   ...
   public:
      TestModuleAsync(dabc::Manager* mgr, const char* name, int kind) : 
...
class TestModuleSync : public dabc::ModuleSync {
   ...
   public:
      TestModuleSync(dabc::Manager* mgr, const char* name, int kind) : 
...
void TestChain(dabc::Manager* mgr, bool isM, int number, int testkind = 0)
{
   for (int n=0;n<number;n++) {
      ...
      if (isM) {
         m = new TestModuleSync(mgr, FORMAT(("Module%d",n)), kind);
         mgr->MakeThreadForModule(m);
      } else {
         m = new TestModuleAsync(mgr, FORMAT(("Module%d",n)), kind);
...
class TimeoutTestModuleAsync : public dabc::ModuleAsync {
   ...
   public:
      TimeoutTestModuleAsync(dabc::Manager* mgr, const char* name) : 
         dabc::ModuleAsync(mgr, name),
         fCounter1(0),
         fCounter2(0)
      {
...
void TestTimers(dabc::Manager* mgr, int number)
{
   for (int n=0;n<number;n++) {
      ...
      m = new TimeoutTestModuleAsync(mgr, FORMAT(("Module%d",n)));
