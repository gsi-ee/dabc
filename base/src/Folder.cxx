...
void dabc::Folder::AddChild(Basic* child) 
{
   ...
   if (!fChilds.has_ptr(child)) {
      ...
      child->_SetParent(GetManager(), GetMutex(), this);
   }
}
...
void dabc::Folder::RemoveChilds()
{
   ...
   while (fChilds.size() > 0) {
      ...
      if (child) child->_SetParent(0,0,0);
...
void dabc::Folder::_SetParent(Manager* mgr, Mutex* mtx, Basic* parent)
{
   dabc::Basic::_SetParent(mgr, mtx, parent);

   for (unsigned n=0; n<fChilds.size(); n++) {
      Basic* child = (Basic*) fChilds.at(n);
      child->_SetParent(mgr, mtx, this);
   }
}
