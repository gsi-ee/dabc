...
void dabc::Command::Reply(Command* cmd, bool res)
{
   ...
   {
//      DOUT1(("Cmd %p Client mutex locked %s", cmd, DBOOL((cmd->fClientMutex ? cmd->fClientMutex->IsLocked() : false))));

      LockGuard guard(cmd->fClientMutex);
      
//      DOUT1(("Reply command %p %s Client %p", cmd, cmd->GetName(), cmd->fClient));
      
      if (cmd->fClient)
         proseccreply = cmd->fClient->_CommandReplyed(cmd, res);

//      DOUT1(("Reply command %p done process %s", cmd, DBOOL(proseccreply)));
   }
