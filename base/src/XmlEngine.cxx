...
namespace dabc {
   ...
   protected:
      ...
      std::istream  *fInp{nullptr};
      const char    *fInpStr{nullptr};
      int            fInpStrLen{0};
...
dabc::XMLNsPointer_t dabc::Xml::NewNS(XMLNodePointer_t xmlnode, const char* reference, const char* name)
{
   ...
   SXmlNode_t* node = (SXmlNode_t*) xmlnode;
   if (!name) name = SXmlNode_t::Name(node);
...
bool dabc::Xml::AddComment(XMLNodePointer_t xmlnode, const char* comment)
{
   ...
   if (!xmlnode || !comment) return false;
                                 ...
                                 const char* charset)
{
   ...
   if (alternate >= 0)
     NewAttr(node, nullptr, "alternate", (alternate > 0) ? "yes" : "no");

   if (title) NewAttr(node, nullptr, "title", title);

   NewAttr(node, nullptr, "href", href);
   NewAttr(node, nullptr, "type", type);

   if (media) NewAttr(node, nullptr, "media", media);
   if (charset) NewAttr(node, nullptr, "charset", charset);
...
const char* dabc::Xml::GetNodeName(XMLNodePointer_t xmlnode)
{
   ...
   return !xmlnode ? nullptr : SXmlNode_t::Name(xmlnode);
}
...
dabc::XMLNodePointer_t dabc::Xml::GetChild(XMLNodePointer_t xmlnode)
{
   ...
   SXmlNode_t* res = !xmlnode ? nullptr :((SXmlNode_t*) xmlnode)->fChild;
   // skip content node
   if (res && (*SXmlNode_t::Name(res) == 0)) res = res->fNext;
...
dabc::XMLNodePointer_t dabc::Xml::GetParent(XMLNodePointer_t xmlnode)
{
   ...
   return !xmlnode ? nullptr : (XMLNodePointer_t) ((SXmlNode_t*) xmlnode)->fParent;
}
...
dabc::XMLNodePointer_t dabc::Xml::GetNext(XMLNodePointer_t xmlnode)
{
   ...
   return !xmlnode ? nullptr : (XMLNodePointer_t) ((SXmlNode_t*) xmlnode)->fNext;
}
...
void dabc::Xml::ShiftToNext(XMLNodePointer_t &xmlnode, bool tonode)
{
   ...
   do {
      xmlnode = !xmlnode ? nullptr : (XMLNodePointer_t) ((SXmlNode_t*) xmlnode)->fNext;
      if (!xmlnode || !tonode) return;
...
bool dabc::Xml::IsEmptyNode(XMLNodePointer_t xmlnode)
{
   ...
   return !xmlnode ? true : (((SXmlNode_t*) xmlnode)->fType != kXML_NODE);
}
...
dabc::XMLDocPointer_t dabc::Xml::ParseString(const char* xmlstring, bool showerr)
{
   ...
   if (!xmlstring || (strlen(xmlstring) == 0)) return nullptr;
...
void dabc::Xml::SaveNode(XMLNodePointer_t xmlnode, XmlOutputStream* out, int layout, int level)
{
   ...
   // add starting spaces
   if (!content && (layout > 0)) out->Put(' ',level);
...
dabc::XMLNodePointer_t dabc::Xml::ReadNode(XMLNodePointer_t xmlparent, XmlInputStream* inp, int& resvalue)
{
   ...
   if (!inp) return nullptr;
   if (!inp->SkipSpaces()) { resvalue = -1; return nullptr; }
   ...
   while (inp->CheckFor("<!--")) {
      ...
      if (commentlen < 0) { resvalue = -10; return nullptr; }
   ...
   if (*inp->fCurrent!='<') {
      ...
      if (!parent || parent->fChild) { resvalue = -2; return nullptr; }
   ...
   if (*inp->fCurrent=='/') {
      ...
      if (len <= 0) { resvalue = -3; return nullptr; }

      if (!parent) { resvalue = -4; return nullptr; }
   ...
   *nameptr = 0; // add 0 to the end
   ...
   do {
      ...
      if (nextsymb=='>') { // this is end of parent node, lets find all childs
         if (!canhaschilds) { resvalue = -11; return nullptr; }
