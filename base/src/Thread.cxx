      ...
      try {
         ...
         if (evnt.GetCode() < Worker::evntFirstSystem) {
            if (evnt.GetCode() < Worker::evntFirstAddOn)
               ...
               EOUT("Get event %u for non-existing addon of worker %u %s in thread %s total worker numbers %u", (unsigned) evnt.GetCode(), (unsigned) itemid, worker->GetName(), GetName(), (unsigned) fWorkers.size());
            } else
