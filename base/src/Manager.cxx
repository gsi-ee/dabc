...
dabc::ApplicationPlugin* dabc::Manager::GetPlugin()
{
   Folder* f = GetPluginFolder(false);
   if ((f==0) || (f->NumChilds()<1)) return 0;
   
   return dynamic_cast<ApplicationPlugin*>(f->GetChild(0));
}
...
void dabc::Manager::DoHaltManager()
{
   ...
   dabc::Folder* df = GetPluginFolder(false);
...
int dabc::Manager::ExecuteCommand(Command* cmd)
{
   ...
   if (cmd->IsName(CommandCreateModule::CmdName())) {
      ...
      dabc::Folder* folder = GetPluginFolder(false);
      if ((folder!=0) && (m==0))
         for (unsigned n=0;n<folder->NumChilds();n++) {
            ApplicationPlugin* plugin = 
               dynamic_cast<dabc::ApplicationPlugin*> (folder->GetChild(n));
...
bool dabc::Manager::DoStateTransition(const char* stcmd)
{
   ...
   ApplicationPlugin* plugin = GetPlugin();
...
bool dabc::Manager::Read_XDAQ_XML_Config(const char* fname, const char* context)
{
   dabc::XmlEngine xml; 
   ...
   dabc::XMLNodePointer_t rootnode = xml.DocGetRootElement(doc);
   ...
   if (!findcontext) {
      EOUT(("Specified context: %s not found", (context ? context : "---")));   
      xml.FreeDoc(doc);
      return false;
   }
