...
//      DOUT1(("Call module stop %s", (m ? m->GetName() : "null")));
   ...
   if (cmd->IsName(CmdCreateInputTransport::CmdName())) {
      ...
      const char* inptype = cmd->GetPar("InputTransportType");
      const char* thrdname = cmd->GetPar("InputTransportThread");
      ...
      if (port==0) {
         ...
         dabc::DataInput* inp = CreateDataInput(inptype, 0);

         if (inp==0) {
            EOUT(("Input object %s not created", inptype));
            cmd_res = cmd_false;
         } else
         if (!inp->Read_Init(cmd, port)) {
            EOUT(("Input object %s cannot be initialized", inptype));
            delete inp;
            cmd_res = cmd_false;
         } else {
            LocalDevice* dev = FindLocalDevice();
            DataIOTransport* tr = new DataIOTransport(dev, port, inp, 0);

            if (thrdname==0) thrdname = port->ProcessorThreadName();

            DOUT1(("Use thread %s for input object", thrdname));
   ...
   if (cmd->IsName(CmdCreateOutputTransport::CmdName())) {
      const char* portname = cmd->GetStr("PortName","");
      const char* outtype = cmd->GetPar("OutputTransportType");
      const char* thrdname = cmd->GetPar("OutputTransportThread");
      ...
      if (port==0) {
         ...
         dabc::DataOutput* out = CreateDataOutput(outtype, 0);

         if (out==0) {
            EOUT(("Output object %s not created", outtype));
            cmd_res = cmd_false;
         } else
         if (!out->Write_Init(cmd, port)) {
            EOUT(("Output object %s cannot be initialized", outtype));
            delete out;
            cmd_res = cmd_false;
         } else {
            LocalDevice* dev = FindLocalDevice();
            DataIOTransport* tr = new DataIOTransport(dev, port, 0, out);

            if (thrdname==0) thrdname = port->ProcessorThreadName();

            DOUT1(("Use thread %s for output object", thrdname));
