...
const char* pex::xmlDMAScatterGatherMode = "PexorDMAScatterGather";    //<  sg mode switch
const char* pex::xmlDMAZeroCopy = "PexorDMAZeroCopy";    //<  sg mode switch
const char* pex::xmlFormatMbs = "PexorFormatMbs";    //<  switch Mbs formating already in transport buffer
const char* pex::xmlSingleMbsSubevt = "PexorSingleMbsSubevent";    //<  use one single subevent for all sfps
const char* pex::xmlMultichannelRequest = "PexorMultiTokenDMA";    //<  enable channelpattern request with combined dma for multiple sfps
const char* pex::xmlAutoTriggerRead = "PexorAutoReadout";    //<  enable automatic readout of all configured token data in driver for each trigger
const char* pex::xmlAutoAsyncRead= "PexorAutoAsync"; //< enable automatic triggerless readout of asynchronous sfp chains in driver
const char* pex::xmlMbsSubevtCrate = "PexorMbsSubcrate";    //<  define crate number for subevent header
const char* pex::xmlMbsSubevtControl = "PexorMbsControl";    //<  define crate number for subevent header
const char* pex::xmlMbsSubevtProcid = "PexorMbsProcid";    //<  define procid number for subevent header

const char* pex::xmlSyncRead = "PexorSyncReadout";    //<  switch readout sync mode
//onst char* pex::xmlParallelRead	= "PexorParallelReadout"; //<  switch readout parallel token mode
const char* pex::xmlTriggeredRead = "PexorUseTrigger";    //<  switch trigger mode
const char* pex::xmlDmaMode = "PexorDirectDMA";    //<  switch between direct dma to host,  or token data buffering in pexor RAM
const char* pex::xmlWaitTimeout = "PexorTriggerTimeout";    //<  specify kernel waitqueue timeout for trigger and autoread buffers


const char* pex::xmlWaitForDataReady = "PexorTokenWaitForDataReady";    //<  token request returns only when frontend has data ready

const char* pex::xmlInitDelay= "PexorInitDelay"; //<  sleep time after board reset until pexor is ready

const char* pex::xmlTrixorConvTime = "TrixorConversionTime";    //<  conversion time of TRIXOR module
const char* pex::xmlTrixorFastClearTime = "TrixorFastClearTime";    //<  fast clear time of TRIXOR module

//const char* pex::xmlExploderSubmem	= "ExploderSubmemSize"; //<  size of exploder submem test buffer

const char* pex::xmlModuleName = "PexorModuleName";    //<  Name of readout module instance
const char* pex::xmlModuleThread = "PexorModuleThread";    //< Name of thread for readout module
const char* pex::xmlDeviceName = "PexorDeviceName";
const char* pex::xmlDeviceThread = "PexorDeviceThread";    //<  Name of thread for readout module

const char* pex::nameReadoutAppClass = "pex::ReadoutApplication";
const char* pex::nameDeviceClass = "pex::Device";
const char* pex::nameTransportClass = "pex::Transport";
const char* pex::nameReadoutModuleClass = "pex::ReadoutModule";
const char* pex::nameInputPool = "PexorInputPool";
const char* pex::nameOutputPool = "PexorOutputPool";

const char* pex::commandStartAcq = "StartAcquisition";
const char* pex::commandStopAcq = "StopAcquisition";
const char* pex::commandInitAcq = "InitAcquisition";

const char* pex::parDeviceDRate = "DeviceReceiveRate";

const char* pex::parDaqRunningState = "PexorAcquisitionRunning";

unsigned int pex::Device::fgThreadnum = 0;

pex::Device::Device (const std::string& name, dabc::Command cmd) :
    dabc::Device (name), fBoard (nullptr), fMbsFormat (true), fSingleSubevent (false), fSubeventSubcrate (0),
...
// TODO fBoard->SetLinkspeed(ls); ls from config
  //////////////
...
///////////////////////OLD ^
    ...
    if ((taken = fBoard->Take_DMA_Buffer (false)) == nullptr)
    {
...
dabc::Transport* pex::Device::CreateTransport (dabc::Command cmd, const dabc::Reference& port)
{
  ...
  DOUT0(
      "~~~~~~~~~~~~~~~~~ pex::Device::CreateTransport port %s isinp %s", portref.ItemName ().c_str (), DBOOL (portref.IsInput ()));
...
int pex::Device::RequestReceiveParallelTokens (dabc::Buffer& buf, uint16_t trigtype)
{
  ...
  int bufflag= (fWaitForDataReady ?  fDoubleBufID[0] | 2 : fDoubleBufID[0]);
  tokbuf = fBoard->RequestReceiveAllTokens(channelmask, bufflag, nullptr,0);
...
int pex::Device::RequestAllTokens (dabc::Buffer& buf, bool synchronous, uint16_t trigtype)
{
  ...
  if ((fMemoryTest && !fSkipRequest) || (!fMemoryTest))
  {
    for (fCurrentSFP = 0; fCurrentSFP < PEX_NUMSFP; ++fCurrentSFP)
    {
      ...
      DOUT3 ("pex::Device::RequestAllTokens gets dma buffer 0x%x for sfp:%d ", tokbuf[fCurrentSFP],
          fCurrentSFP);
...
int pex::Device::ReceiveAllTokenBuffer (dabc::Buffer& buf, uint16_t trigtype)
{
  ...
  for (int sfp = 0; sfp < PEX_NUMSFP; ++sfp)
  {
    ...
    if ((fMemoryTest && !fSkipRequest) || !fMemoryTest)
    {
      tokbuf[sfp] = fBoard->WaitForToken (sfp, fDirectDMA, nullptr, 0, IsSynchronousRead()); // error messages supressed for sync read
...
int pex::Device::CopyOutputBuffer (pexor::DMA_Buffer* tokbuf, dabc::Buffer& buf, uint16_t trigtype)
{
  ...
  {
    EOUT(
        "Token buffer used size %ld + header sizes %d exceed available target buffer length %d \n", tokbuf->UsedSize (), used_size, buf.GetTotalSize ());
    EOUT(
        "Mbs Event header size is %ld;  Mbs subevent header sizes: %ld \n", sizeof(mbs::EventHeader), sizeof(mbs::SubeventHeader));
...
int pex::Device::CopySubevent (pexor::DMA_Buffer* tokbuf, dabc::Pointer& cursor, char sfpnum)
{
  ...
  DOUT2 ("pex::Device::CopySubevent has dma buffer %p for sfp %d, output cursor pointer :%p", tokbuf,
  ...
  DOUT2 ("---------- token used size :%ld", tokbuf->UsedSize ());DOUT2 ("---------- filledsize :%d", filled_size);

  return filled_size;
}

mbs::EventHeader* pex::Device::PutMbsEventHeader (dabc::Pointer& ptr, mbs::EventNumType eventnumber,
    uint16_t trigger)
{
  ...
  {
    DOUT0(
        "pex::Device::PutMbsEventHeader fails because no more space in buffer, restsize=%d bytes", ptr.rawsize());
    return nullptr;
  }
    ...
    int16_t procid)
{
  ...
  {
    DOUT0(
        "pex::Device::PutMbsSubeventHeader fails because no more space in buffer, restsize=%d bytes", ptr.rawsize());
    return nullptr;
  }
...
// TODO: check if padding words would exceed buffer length.
  ...
  {
    DOUT0(
        "pex::Device::PutMbsPaddingWords fails because no more space in buffer, restsize=%d bytes, paddingwords=%d", ptr.rawsize(), num);
    return -1;
  }
    ...
    default:
      ...
      if (!IsAutoReadout ())    // only request further data if we do not have already automatically filled buffer
      {
        if (IsMultichannelMode ())
        {
          ...
          {
            EOUT(
                "pex::Device::  PexorMultiTokenDMA Mode does not work with direct DMA! Please change config.\n");
            exit (1);
          }
...
double pex::Device::ProcessTimeout(double /* last_diff */)
{
  ...
  DOUT3("pex::Device::ProcessTimeout with dt= %f s - acuisition state=%d, parameter=%d!!", last_diff, fAqcuisitionRunning, Par(pex::parDaqRunningState).Value().AsBool());
