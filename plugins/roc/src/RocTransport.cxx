...
roc::RocTransport::RocTransport(roc::RocDevice* dev, dabc::Port* port, int bufsize, const char* boardIp, int trWindow) :
   ...
   fReqNumMsgs(0)
{     
   DOUT1(("RocTransport constructor"));

   port->AssignTransport(this);
   
   // create board object:
   int id = fxRocDevice->AddBoard(boardIp, true);
   if(id<0) 
      EOUT(("!!! ROCTransport: failed to add board %s", boardIp));
   else {
      fuBoardId = (unsigned) id; 
      fxBoard = fxRocDevice->GetBoard(fuBoardId);
   }
   ...
   DOUT5(("RocTransport constructor done"));

}

roc::RocTransport::~RocTransport()
{
   DOUT5(("RocTransport destructor."));
}


void roc::RocTransport::StartTransport()
{
   if(fxBoard==0) {
   ...
   DOUT1(("RocTransport::StartTransport().., using board id:%d, hostport:%u",fuBoardId,fxBoard->getHostDataPort()));
...
void roc::RocTransport::StopTransport()
{
   DOUT5(("RocTransport::StopTransport()..."));
   fxBoard->stopDaq();
   dabc::DataTransport::StopTransport(); 
}

unsigned roc::RocTransport::Read_Size()
{
   return fBufferSize;
}

unsigned roc::RocTransport::Read_Start(dabc::Buffer* buf)
{
...
unsigned roc::RocTransport::Read_Complete(dabc::Buffer* buf)
{
...
void roc::RocTransport::ComplteteBufferReading()
{
