...
SysCoreSorter::SysCoreSorter(unsigned max_inp_buf, unsigned intern_out_buf, unsigned intbuf)
{
   fRocId = 0;
   ...
   if (sizeof(SysCoreData) != 6) {
      printf("SysCoreSorter will not work on platform where sizeof(SysCoreData) = %u\n", sizeof(SysCoreData));
      exit(1);
   }
   
   fIntBuf = (SysCoreData*) malloc(fIntBufMaxSize*6);
   ...
   fFillBuffer = (SysCoreData*) fInternOutBuf; 
...
bool SysCoreSorter::startFill(void* buf, unsigned totalsize)
{
   if (fFillSize!=0) {
   ...
   if (totalsize < 243*6) {
      printf("SysCoreSorter::startFill provided buffer size %u too small\n", totalsize);
      return false;
   }
   
   fFillBuffer = (SysCoreData*) buf;
...
void SysCoreSorter::cleanBuffers()
{
...
bool SysCoreSorter::addData(SysCoreData* new_data, unsigned num_msg, bool flush_data)
{
...
//               if (data->getNxTs() > 0x3fe0) printf("CANBE REALLY???\n");
            ...
            if (fIntBufCurrEpoch > 0) {
               ...
               if (indx==0) 
                  ...
                  while (revindx-->0) {
                     SysCoreData* rev = revindx < fIntBufSize ? 
                                             (fIntBuf + revindx) :
...
bool SysCoreSorter::flushBuffer(SysCoreData* new_data, unsigned num_msg, bool force_flush)
{
   ...
   SysCoreData *src_data(0), *out_data(fFillBuffer + fFillSize);
...
bool SysCoreSorter::shiftFilledData(unsigned num)
{
   if (num>=fFillSize) {
      fFillSize = 0; 
      return true;
