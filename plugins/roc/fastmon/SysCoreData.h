#ifndef SYSCOREDATA_H_
#define SYSCOREDATA_H_
...
class SysCoreData
{
   ...
   public:
      ...
      SysCoreData();
      SysCoreData(const SysCoreData& src);
      ...
      // sync message time stamp resolution 2 ns, 13 bit
      ...
      // lower bit of epoch when AUX was taken
...
//                ((data[3] & 0xe0) >> 5);
      ...
      inline uint8_t getAuxFailing() const
      {
         return (data[3] >> 4) & 1;
      }
...
//         data[3] = (data[3] & ~0xe0) | (v << 5);
      ...
      inline uint64_t getMsgFullTime(uint32_t epoch) const
      {
         switch (getMessageType()) {
            ...
            case ROC_MSG_AUX:
               return FullTimeStamp(epoch, getAuxTs());
         } 
         return 0;
      }
