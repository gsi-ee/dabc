...
MainUserAnalysis and TRocFastAnalysis
...
For each event the user event processor TRocFastProc is
called. This user event processor fills some histograms
and an output event TRocFastEvent (raw event) from the input event.
...
that the viewer knows TRocFastEvent.

All classes are defined and declared in two files (*.h and *.cxx)
In TRocFastAnalysis the analysis step is created with the step factory and input and output
parameters. Here the defaults are set concerning the event IO.
Two parameter objects are created (TRocFastParam and TRocFastControl).

The event filled: TRocFastEvent
The processor:    TRocFastProc

The factory created in TRocFastAnalysis keeps all information about the step. The TRocFastEvent
contains the data members to be filled in TRocFastProc from the input event (MBS 10,1).
The Clear method must clear all these members (an array for each crate in the example).
The analysis code is in the event processor TRocFastProc. Members are
histograms, conditions, and parameter pointers used in the event method
Event. In the constructor of TRocFastProc the histograms and
conditions are created, and the pointers to the parameter objects (created in
TRocFastAnalysis) are retrieved. Function BuildEvent - called event by event - gets the output
...
output event TRocFastEvent. Two subevents (crate 1,2) are processed.
Then the histograms are filled, the 2d one with polygon conditions.

A Parameter class TRocFastParam
In this class one can store parameters, and use them in all steps.
Parameters can be modified from GUI.
Parameter class TRocFastControl
This class has one member "fill" which is checked in TRocFastProc->Event
...
is actually used to set filling on or off (in TRocFastProc). You can modify histofill.C
by editor.
...
from that file.
When TRocFastAnalysis is created, the autosave file is not yet loaded. Therefore the
...
Adapt the example
Most probably you will change TRocFastParam to keep useful parameters.
Then you might change TRocFastEvent to represent your event data.
Keep the Clear method consistent with the data members!
Then definitely you will change TRocFastProc to create your histograms, conditions,
pictures, and finally write your analysis function BuildEvent.

In TRocFastAnalysis there are three more functions which eventually can be useful:
UserPreLoop (called before event loop starts),
UserEventFunc (called after TRocFastProc->Event),
