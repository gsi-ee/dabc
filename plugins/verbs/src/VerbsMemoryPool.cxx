...
dabc::VerbsMemoryPool::VerbsMemoryPool(VerbsDevice* verbs,
                                       const char* name,
                                       int32_t number, 
                                       int64_t bufsize, 
                                       bool isud,
   ...
   f_sge(0)
{
   ...
   fReg = new VerbsPoolRegistry(verbs, this, true);
...
dabc::VerbsMemoryPool::~VerbsMemoryPool()
{
   delete fReg; 
...
uint32_t dabc::VerbsMemoryPool::GetLkey(BufferId_t id)
{
   return fReg ? fReg->GetLkey(id) : 0;
}

struct ibv_recv_wr* dabc::VerbsMemoryPool::GetRecvWR(BufferId_t bufid)
{
...
struct ibv_send_wr* dabc::VerbsMemoryPool::GetSendWR(BufferId_t bufid, uint64_t size)
{
   // special case, when working without Buffer objects at all
...
void* dabc::VerbsMemoryPool::GetSendBufferLocation(BufferId_t bufid)
{
