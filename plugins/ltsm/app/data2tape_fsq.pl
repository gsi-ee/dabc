...
my $opt_arch; # jul18
my $opt_ltsmserv = "lxfsq09";
my $opt_ltsmfs   = "/lustre";

my $opt_ltsmnode = "hades";
my $opt_ltsmpass; 
my $opt_ltsmdest = "lustre_tsm";
# lustre_tsm
my $opt_daypath ="/23/159/";
...
sub help()
{
    ...
    print << 'EOF';
date2tape_fsq.pl
 -- adjusted for FSQ JAM 12-Jun-2023
...
Usage:
   ...
   [-a|--arch <archive>]     : Specify archive name. MANDATORY for real archiving!
   [-w|--password <pw>]      : TSM server password.
   [-d|--daypath <subpath>]  : daily subpath (e.g. /23/159).

Examples:

   Archive all hld files with prefixes 'st' and 'be' from sept 4th, 2018 for beamtime jul18:
      date2tape_fsq.pl -p be -p st -s 2018-09-04_00:00:00 -e 2018-09-04_23:59:59 -a jul18 -w <secret>

   Archive all hld files from the list /home/hadaq/oper/diskfiles.txt to archive "jul18":
      date2tape_fsq.pl -a may22test  -f /home/hadaq/oper/diskfiles.txt -w <secret>
...
sub archive()
{
	...
	my $cmd  = "cat \"$hldfile\" | fsqc --pipe -n \"$opt_ltsmnode\" ";
	$cmd .= "-f \"$opt_ltsmfs\" -p \"$opt_ltsmpass\" ";
	$cmd .= "-s \"$opt_ltsmserv\" -o \"$opt_ltsmdest\" ";
	my $reduced_file= fileparse($hldfile);
	$cmd .= "-a \"$opt_ltsmfs$ltsmpath\" -l \"$reduced_file\"";
	
	print "archiving $reduced_file with fsqcc at $hms..\n";		
