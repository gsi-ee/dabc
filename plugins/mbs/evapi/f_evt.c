...
#undef unix
#include "s_filhe.h"
#include "s_bufhe.h"   /* GOOSY buffer header */
#include "s_ve10_1.h"  /* GOOSY event header          */
#include "s_ves10_1.h"  /* GOOSY event header          */
#include "s_evhe.h"    /* GOOSY spanned event header  */
#endif
#ifdef GSI__LINUX   /* Linux */
#undef unix
#include "s_filhe_swap.h"
#include "s_bufhe_swap.h"   /* GOOSY buffer header */
#include "s_ve10_1_swap.h"  /* GOOSY event header          */
#include "s_ves10_1_swap.h"  /* GOOSY event header          */
...
static l_gl_rev_port=PORT__EVENT_SERV;
static l_gl_evt_check=0;
...
if(ps_bufhe != NULL)
{
  ...
  if(ps_bufhe->i_type == 2000)
  {
    ...
    printf("%s\n",c_line);
    sprintf(c_line,"Size:    %d [%d b]",ps_filhe->filhe_dlen,ll);
    printf("%s\n",c_line);
  ...
  {
    sprintf(c_line,"Buffer  %9d, Length %5d[w] Size %5d[b] %s",
        ps_bufhe->l_buf,
        ps_bufhe->l_dlen,
       ll,
        c_time);
   ...
   CHARS **ps_info, INTS4 l_sample, INTS4 l_param)
{
   ...
   INTS4 l_swap, l_swap_head, l_is_goosybuf, l_filehead=0, l_size, l_size_head, l_dummy;
   INTS2 *pi;
   CHARS c_file[80], *pc_temp;
   s_filhe *ps_filhe;
   struct s_varstr
   {
     INTS2 i_varstr;
   ...
   ps_chan->l_timeout=-1; /* no timeout */
     ...
     if((l_status&1) != 1)
    {
     ...
     l_status = i$clilf(&ps_chan->l_channel_no);
       ...
       if(l_filehead == 1) {
         ...
         if(read(ps_chan->l_channel_no,c_temp,l_size_head)!=l_size_head) {
           printf("LMD format error: no LMD file: %s\n",c_file);
           close(ps_chan->l_channel_no);
           ps_chan->l_channel_no=-1;
           return(GETEVT__NOLMDFILE);
...
{
   INTS4 l_temp,l_prev_ok=1, l_stat;
   ...
   while(1)
   {
      ...
      while (ps_chan->l_buf_posi >= ps_chan->l_buf_lmt)
      {
         ...
         if(ps_chan->ps_bufhe->i_type == 2000) {   /* file header */
            ...
            ps_chan->l_buf_lmt = ps_chan->l_buf_posi + (ps_chan->ps_bufhe->i_used)*2;
            ps_chan->l_io_buf_posi += ps_chan->l_buf_size;
         }
