...
mbs::MbsServerPort::MbsServerPort(MbsServerTransport* tr, int serversocket, int portnum) : 
...
mbs::MbsServerIOProcessor::MbsServerIOProcessor(MbsServerTransport* tr, int fd) :
...
mbs::MbsServerTransport::MbsServerTransport(MbsDevice* dev, dabc::Port* port, 
                                            int kind, 
                                            int serversocket, 
                                            int portnum,
                                            uint32_t maxbufsize) :
...
mbs::MbsServerTransport::~MbsServerTransport()
{
   DOUT1(("mbs::MbsServerTransport::~MbsServerTransport queue:%d", fOutQueue.Size())); 
   ...
   DOUT1(("mbs::MbsServerTransport::~MbsServerTransport done queue:%d", fOutQueue.Size())); 
}

void mbs::MbsServerTransport::PortChanged() 
{
...
void mbs::MbsServerTransport::ProcessConnectionRequest(int fd)
{
   if (fIOSocket!=0) {
...
void mbs::MbsServerTransport::SocketIOClosed(MbsServerIOProcessor* proc)
{
   bool testdrop = false;
   
   {
      ...
      DOUT1(("MbsServerTransport send:%ld dropped:%ld", fSendBuffers, fDroppedBuffers));
...
dabc::Buffer* mbs::MbsServerTransport::TakeFrontBuffer()
{
   dabc::Buffer* buf = 0;
...
void mbs::MbsServerTransport::DropFrontBufferIfQueueFull()
{
   dabc::Buffer* buf = 0;
...
bool mbs::MbsServerTransport::Send(dabc::Buffer* buf)
{
   bool res = false;
...
unsigned mbs::MbsServerTransport::SendQueueSize()
{
   dabc::LockGuard guard(fMutex); 
