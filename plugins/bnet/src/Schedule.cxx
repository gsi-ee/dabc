...
bnet::IBClusterRouting::IBClusterRouting()
{
   fNumSpines = 12;

   for (int n1=0;n1<MaxNumNodes;n1++) {
      fMatrix[n1] = new IBRoute* [MaxNumNodes];
      for (int n2=0;n2<MaxNumNodes;n2++)
         fMatrix[n1][n2] = new IBRoute[MaxNumLids];
   }
...
void bnet::IBClusterRouting::Reset()
{
   fNumNodes = 0;
   fNumLids = 1;
...
int bnet::IBClusterRouting::FindNode(const std::string& name)
{
...
int bnet::IBClusterRouting::AddNode(const std::string& name, int lid, int* liddiff)
{
   NodesMap::iterator iter = fNodes.find(name);
...
int bnet::IBClusterRouting::AddSwitch(const std::string& name)
{
...
int bnet::IBClusterRouting::FindSwitch(const std::string& name)
{
   NamesMap::iterator iter = fSwitchNames.find(name);
...
void bnet::IBClusterRouting::AddCSCSwitches(int nspines, int nleafs)
{
   for (int n=1;n<=nspines;n++)
      AddSwitch( dabc::format("ibspine%02d",n) );
   for (int n=1;n<=nleafs;n++)
      AddSwitch( dabc::format("ibswitch%02d",n) );

   fNumSpines = nspines;
}


bool bnet::IBClusterRouting::ReadFile(std::string filename)
{
   Reset();
...
void bnet::IBClusterRouting::ReinjectOptimizedRoutes()
{
   for (int n1=0; n1<NumNodes(); n1++) {
      ...
      for (int n2=0; n2<NumNodes(); n2++) {
         ...
         IBRoute r12 = GetRoute(n1, n2);
         if (r12.GetNumHops()!=1) continue;

         IBRoute r21 = GetRoute(n2, n1);
         ...
         for (int n3=0; n3<NumNodes(); n3++) {
            ...
            IBRoute r13 = GetRoute(n1, n3);

            if (r13.IsNull()) continue;

            IBRoute r23 = GetRoute(n2, n3);
...
bool bnet::IBClusterRouting::CheckTargetRoutes()
{
         ...
         for (IntMap::iterator iter2 = iter1; iter2!=nodes.end(); iter2++) {
             ...
             for (int node=0;node<NumNodes();node++) {
                ...
                for (int lid=0; lid<NumLids(); lid++) {
                   ...
                   IBRoute r1 = GetRoute(node1, node, lid);
                   IBRoute r2 = GetRoute(node2, node, lid);
...
int bnet::IBClusterRouting::CheckUsefulLIDs()
{
...
void bnet::IBClusterRouting::GenerateFullTopology(int switchsize, bool manylids)
{
   ...
   for (int n=0;n<NumLeafs();n++)
      for (int m=0;m<nodesperswitch;m++)
        AddNode( dabc::format("node%02d-%02d", n + 1, m + 1) );
...
const char* bnet::IBClusterRouting::NodeName(int nodeid) const
{
...
const char* bnet::IBClusterRouting::SwitchName(int id) const
{
...
bnet::IBRoute bnet::IBClusterRouting::GetRoute(int node1, int node2, int lid) const
{
   IBRoute res;
...
std::string bnet::IBClusterRouting::GetRouteAsString(int node1, int node2, int lid) const
{
   IBRoute res = GetRoute(node1, node2, lid);
...
void bnet::IBClusterRouting::ExcludeNode(int nodeid)
{
...
void bnet::IBClusterRouting::RemoveUndefinedRoutes()
{
...
void bnet::IBClusterRouting::BuildOptimalRoutes()
{
...
bool bnet::IBClusterRouting::MatchNodes(IBClusterRouting& other)
{
   int n1 = 0;
   int n2 = 0;
...
void bnet::IBClusterRouting::PrintDifferecne(IBClusterRouting& other)
{
   DOUT0("======================");
...
void bnet::IBClusterRouting::FindSameRouteTwice(bool bothlanes, const IBRoute& route0, bool rnd)
{
   std::map<IBRoute, int> routes;
   ...
   for (int in1=0;in1<NumNodes();in1++)
      for (int in2=0;in2<NumNodes();in2++) {
         ...
         IBRoute route1 = GetRoute(n1, n2);

         IBRoute route2, rev_route1, rev_route2;
...
int bnet::IBClusterRouting::GetNodeSwitch(int node)
{
   if ((node<0) || (node>NumNodes()) || (NumNodes()<2)) return -1;

   IBRoute route = GetRoute(node, node==0 ? 1 : 0);
...
bnet::IBSchedule::IBSchedule(int MaxNumSlots, int NumSenders)
{
  fSchedule = 0;
  fTimeSlots = 0;
...
void bnet::IBSchedule::Allocate(int maxnumslots, int numsend)
{
   if (fSchedule!=0) {
   ...
   if ((fNumSenders>0) && (fMaxNumSlots>0)) {
      fSchedule = new IBScheduleItem* [fMaxNumSlots];
      for(int n=0;n<fMaxNumSlots;n++)
         fSchedule[n] = new IBScheduleItem[fNumSenders];
...
void bnet::IBSchedule::Clear()
{
   if (numSlots()==0) return;

   for (int nslot=0; nslot<numSlots(); nslot++) {
      IBScheduleItem* slot = getScheduleSlot(nslot);
...
void bnet::IBSchedule::FillReceiveSchedule(IBSchedule& recv)
{
   ...
   for (int nslot=0; nslot<numSlots(); nslot++) {
      IBScheduleItem* slot = getScheduleSlot(nslot);
      IBScheduleItem* slot2 = recv.getScheduleSlot(nslot);
...
bool bnet::IBSchedule::IsNodeActive(int node)
{
...
void bnet::IBSchedule::ExcludeInactiveNode(int node)
{
...
bool bnet::IBSchedule::BuildOptimized(IBClusterRouting& routing, dabc::IntColumn* ids, bool show)
{
   ...
   while (numSlots() < maxNumSlots()) {
      ...
      IBScheduleItem* slot = getScheduleSlot(fNumSlots++);
      if (slot==0) {
         EOUT("No more slots");
         return false;
      }
      ...
      for (int nsend = 0; nsend<numids; nsend++) {
         ...
         for (int nrecv = 0; nrecv < numids; nrecv++) {
            ...
            for (int lid=0;lid<routing.NumLids();lid++) {
               ...
               IBRoute route = routing.GetRoute(sender, receiver, lid);
...
bool bnet::IBSchedule::BuildRegularSchedule(IBClusterRouting& routing, dabc::IntColumn* ids, bool show)
{
   ...
   for (int id=0; id<numids; id++) {
      ...
      IBRoute route = routing.GetRoute(node, othernode);
   ...
   for (int nslot=0; nslot < regular_num_slots; nslot++) {
      ...
      for (int nsend = 0; nsend < regular_numnodes; nsend++) {
         ...
         if (nrecv!=nsend) {
            ...
            for (int lid=0;lid<routing.NumLids();lid++) {
               IBRoute route = routing.GetRoute(code_nsend, code_nrecv, lid);
   ...
   for (int nslot=0; nslot < regular_num_slots; nslot++) {
      ...
      for (int nsend = 0; nsend < numSenders(); nsend++) {
         ...
         for (int nslotnew = regular_num_slots; nslotnew<numSlots(); nslotnew++) {
            IBScheduleItem* newslot = getScheduleSlot(nslotnew);
            ...
            for (int nsend1 = 0; nsend1 < numSenders(); nsend1++) {
               ...
               IBRoute route = routing.GetRoute(nsend1, newslot[nsend1].node, newslot[nsend1].lid);
            ...
            for (int lid=0;lid<routing.NumLids();lid++) {
               IBRoute route = routing.GetRoute(nsend, slot[nsend].node, lid);
         ...
         if (need_new_slot) {
            ...
            IBScheduleItem* newslot = getScheduleSlot(fNumSlots++);
            if (newslot==0) return false;
...
void bnet::IBSchedule::PrintSlotWithRouting(const IBClusterRouting& routing, int nslot)
{
...
void bnet::IBSchedule::PrintWithRouting(const IBClusterRouting& routing, int nlast)
{
...
bool bnet::IBSchedule::CanMoveItemTo(const IBClusterRouting& routing, int nsender, int nslot1, int nslot2, int& newlid)
{
...
//         ((nslot1==841) && (nsender==642));
   ...
   for (int lid=0; lid<routing.NumLids(); lid++) {
      ...
      IBRoute route = routing.GetRoute(nsender, nreceiver, lid);
      ...
      IBScheduleItem* slot2 = getScheduleSlot(nslot2);
      ...
      for (int nsend=0;nsend<numSenders();nsend++) {
         ...
         IBRoute rrr = routing.GetRoute(nsend, slot2[nsend].node, slot2[nsend].lid);
...
bool bnet::IBSchedule::RollBack(IBScheduleMoveList& lst)
{
   while (lst.size()>0) {
      IBScheduleMove oper = lst.back();
      lst.pop_back();
...
void bnet::IBSchedule::MoveSender(IBScheduleMoveList& lst, int nsender, int src_slot, int tgt_slot, int lid)
{
   lst.push_back(IBScheduleMove(nsender, src_slot, tgt_slot, lid>=0 ? Item(src_slot, nsender).lid : -1));
...
bool bnet::IBSchedule::TryMoveSender(const IBClusterRouting& routing, IBScheduleMoveList& global_lst, int nsender0, int nslot0, int nslot1, int routekind, int nslot2, bool show)
{
   ...
   IBScheduleItem* slot0 = getScheduleSlot(nslot0);
   IBScheduleItem* slot1 = getScheduleSlot(nslot1);
   ...
   if (nsender1>=0) {
      ...
      IBRoute route1 = routing.GetRoute(nsender1, nreceiver0, 0);
...
//   }
   ...
   for (int lid0=0; lid0<routing.NumLids(); lid0++) {
      ...
      IBRoute route0 = routing.GetRoute(nsender0, nreceiver0, lid0);
      ...
      for (int nsend=0; nsend<numSenders(); nsend++) {
         ...
         IBRoute rrr = routing.GetRoute(nsend, slot1[nsend].node, slot1[nsend].lid);
...
bool bnet::IBSchedule::TryToCompress(IBClusterRouting& routing, bool show, double runtime)
{
   ...
   while ((numSlots() > 0) && (nslot0>=0)) {
      ...
      if (nsender0 < 0) {
         ...
         for (int n1 = nslot0; n1 < fNumSlots - 1; n1++) {
            IBScheduleItem* sl0 = getScheduleSlot(n1);
            IBScheduleItem* sl1 = getScheduleSlot(n1+1);
...
double bnet::IBSchedule::CheckConjunction(IBClusterRouting& routing, bool show)
{
   ...
   for(int nslot=0;nslot<numSlots();nslot++) {
      ...
      for(int nsend=0;nsend<numSenders();nsend++) {
         ...
         IBRoute route = routing.GetRoute(nsend, nrecv, lid);
...
bool bnet::IBSchedule::RecodeIds(const dabc::IntColumn& ids, IBSchedule& new_sch)
{
   ...
   for (int nslot=0;nslot<numSlots();nslot++) {
      ...
      IBScheduleItem* slot = getScheduleSlot(nslot);
      IBScheduleItem* new_slot = new_sch.getScheduleSlot(nslot);
...
bool bnet::IBSchedule::CopyTo(IBSchedule& new_sch)
{
...
void bnet::IBSchedule::FillRegularTime(double schedule_step)
{
   for (int nslot=0;nslot<numSlots();nslot++)
...
bool bnet::IBSchedule::SaveToFile(const std::string& fname)
{
   FILE *f = fopen (fname.c_str(), "w");
...
bool bnet::IBSchedule::ReadFromFile(const std::string& fname)
{
   char sbuf[100];
...
void BuildConjunctionCurve(bnet::IBClusterRouting& routing)
{
   ...
   for (int num = 50; num <= maxnum; num+=50) {
      ...
      for (int ntest=0;ntest<10;ntest++) {
         ...
         bnet::IBSchedule sch1(routing.NumNodes()-1, routing.NumNodes());
...
extern "C" void CompareRouting()
{
   ...
   for (unsigned n=0; n<arr.size()-1;n++) {
      ...
      bnet::IBClusterRouting routing1, routing2;
...
extern "C" void TestSorting()
{
   ...
   bnet::IBSchedule sch2(routing.NumNodes()*3, routing.NumNodes());
...
//   routing.FindSameRouteTwice(routing.GetRoute(100,200));
   ...
   routing.FindSameRouteTwice(false, bnet::IBRoute(), true);
   ...
   for (int n=0;n<4;n++) {
      ...
      bnet::IBSchedule sch2(ids.size()*3, ids.size());
      sch2.BuildOptimized(routing, &ids, true);
      sch2.CheckConjunction(routing, true);
...
extern "C" void SelectCSCNodes()
{
   ...
   if (all_args=="schedule") {
      ...
      bnet::IBSchedule sch3(routing.NumNodes()*3, routing.NumNodes());
